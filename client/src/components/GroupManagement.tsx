                              ...
                              {(subgroup.members || []).length > 0 ? (
                                <div className="space-y-2">
                                  {(subgroup.members || []).map((member: any) => {
                                    ...
                                    const role = userData.role || userFromList?.role
                                    const isTeamLead = role === 'team_lead' || subgroup.teamLeadId === memberUserId
                                    const memberPermissions = member.memberPermissions || []

                                    return (
                                      <div key={memberUserId} className="p-3 bg-slate-50 dark:bg-slate-900 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                                        ...
                                        {/* Member's Individual Permissions */}
                                        {memberPermissions.length > 0 && (
                                          <div className="ml-10 mt-2 pt-2 border-t border-slate-200 dark:border-slate-700">
                                            <p className="text-xs font-semibold text-muted-foreground mb-1">Member Permissions:</p>
                                            <div className="space-y-1">
                                              {memberPermissions.map((perm: any, idx: number) => (
                                                <div key={idx} className="flex items-center justify-between">
                                                  <span className="text-xs font-medium">{perm.resource}</span>
                                                  <div className="flex flex-wrap gap-1">
